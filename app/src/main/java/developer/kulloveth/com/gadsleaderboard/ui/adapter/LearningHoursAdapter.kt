...
class LearningHoursAdapter:ListAdapter<LearnerByHour,LearningHoursAdapter.LearningHourViewHolder>(
    HOUR_DIFF) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LearningHourViewHolder {
       val binding = RvItemLayoutBinding.inflate(LayoutInflater.from(parent.context),parent,false)
    ...
    inner class LearningHourViewHolder(private val binding:RvItemLayoutBinding):RecyclerView.ViewHolder(binding.root){
        fun bindView(hour: LearnerByHour){
           binding.durationTv.text = hour.hours.toString()
